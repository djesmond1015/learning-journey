...
export const getQuestionFromTranscript = async (
  transcript: string,
  course_title: string
) => {
  type Question = {
  ...
  const questions: Question[] = await strict_output(
    ...
    new Array(5).fill(
      `You are to generate a random hard mcq question about ${course_title} with context of the following: ${transcript} `
    ),
    {
      question: 'question',
